...
async function publishOrderCompletedEvent(orderData) {
  ...
  try {
    ...
    console.log(`[RabbitMQ AMQP] Broker connection unreached (${error.message}). Falling back to HTTP Webhook...`);
  }

  // Webhook dispatch to Notification Service for instant delivery & fallback
  try {
    ...
    console.log(`[Order Service Webhook] Event delivered to Notification Service. Status: ${response.status}`);
  } catch (httpError) {
    console.log(`[Order Service Webhook] Direct notification delivery note: ${httpError.message}`);
  }
}
