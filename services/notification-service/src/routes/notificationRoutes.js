...
router.post('/event', async (req, res) => {
  try {
    ...
    if (eventType === 'ORDER_COMPLETED' && data) {
      const orderId = data._id ? data._id.toString() : (data.orderId || `ord_${Date.now()}`);
      const { userId, customerName, customerEmail, subtotal, tax, deliveryFee, totalAmount, items } = data;

      // Deduplication check: return existing notification if already created
      const existingNotif = await Notification.findOne({ orderId: orderId || `ord_${Date.now()}` });
      if (existingNotif) {
        console.log(`[Notification Service] Notification for Order #${orderId} already exists. Returning existing.`);
